...
/**
 * Ensures all BidIQ tables exist. Runs automatically before sync.
 * Uses POSTGRES_URL_NON_POOLING or POSTGRES_URL for direct DDL.
 */
export async function ensureTables(): Promise<void> {
  const url = POSTGRES_URL;
  if (!url) {
    throw new Error(
      "Missing POSTGRES_URL or POSTGRES_URL_NON_POOLING. Add from Supabase Dashboard > Settings > Database."
    );
  }
  ...
  try {
    ...
    // Run migration in one transaction; use DO block for ALTER PUBLICATION to avoid re-run errors
    const sql = `
-- Tables
